...
GENES: tuple[CapabilityGene, ...] = (
    ...
    G(54,"Durable Async Job Handle / Poll-Push Continuation","DURABILITY",("OpenAI Background Responses","A2A","Microsoft Durable Task"),HarvestMode.COMPOSED_BY_FABRIC,"frontier_resilience_interop_v1.reconcile_async_job + FDOF/DeliveryJournal","client disconnect preserves one task/result identity; poll/push/readback cannot duplicate effects"),
    G(55,"Exact Run-State / Pending-Batch Reconciliation","DURABILITY",("OpenAI Agents RunState","Temporal","Microsoft Durable Task"),HarvestMode.REUSE_VERIFIED,"frontier_resilience_interop_v1.exact_resume_gate + FDOF checkpoint","resume committed tool batch without re-execution; ambiguous history fails closed"),
    G(56,"TTL / Cache-Scope Tool Catalogue Currentness","TOOLS",("MCP 2026","Federation Capability Market","OpenAI/Anthropic tool discovery"),HarvestMode.COMPOSED_BY_FABRIC,"frontier_resilience_interop_v1.tool_catalogue_cache_decision + lazy tool registry","cached tool/resource/schema list reused only inside declared TTL/scope and invalidated on capability/auth epoch"),
    G(57,"Stateful Agent Task / Artifact Resubscribe & Push","ORCHESTRATION",("A2A","OpenAI Background Responses","Google Agent Engine Sessions"),HarvestMode.COMPOSED_BY_FABRIC,"frontier_resilience_interop_v1.task_delivery_plan + Mission Bus/A2A adapter","stateful task survives disconnect; resubscribe/push returns same task/artifact identity and terminal state"),
    G(58,"Browser Offscreen Liveness / Singleflight Context","EXECUTION",("Chrome Offscreen API","Chrome runtime.getContexts","ChatBridge/BEF"),HarvestMode.BUILD_ADDITIVE,"frontier_resilience_interop_v1.offscreen_context_plan + ChatBridge/BEF receiver","service-worker restart recreates at most one helper; no duplicate writer/effect and current context is observable",2),
    G(59,"Replay-Safe Durable Workflow Versioning","DURABILITY",("Temporal","Microsoft Durable Task","Google ADK durable integration"),HarvestMode.COMPOSED_BY_FABRIC,"frontier_resilience_interop_v1.workflow_version_gate + SOL6.2/FDOF history","restart/redeploy replays deterministic history without repeating completed effects; incompatible version fails closed"),
    G(60,"Resource-Adaptive Worker Slots / Poller Autoscaling","ROUTING",("Temporal","Google Agent Runtime","Federation Execution Power Pools"),HarvestMode.COMPOSED_BY_FABRIC,"frontier_resilience_interop_v1.adaptive_worker_slots + Execution Power Pools","concurrency follows backlog/CPU/memory/SLO without oversubscription, authority widening or starvation"),
    G(61,"Local Responses / Tools / MCP Runtime Compatibility","EXECUTION",("LM Studio","LocalLLM","OpenAI-compatible protocol"),HarvestMode.BUILD_ADDITIVE,"frontier_resilience_interop_v1.local_openai_compatibility_gate + LocalLLM/OmniSurface","local route supports model discovery, Responses/tool calls and MCP/stateful session contract under privacy/offline court",2),
    G(62,"Failure-Domain Placement / Disruption Budget","ROUTING",("Kubernetes topology spread","Kubernetes disruption budget","FUSE Portfolio V2"),HarvestMode.COMPOSED_BY_FABRIC,"frontier_resilience_interop_v1.failure_domain_placement + Portfolio V2/min-cut reducer","redundant routes are spread across proven independent domains and planned disruption cannot remove all terminal-critical capacity"),
    G(63,"OAuth Issuer / Dynamic-Client Mix-Up Hardening","SECURITY",("MCP 2026 OAuth","RFC 9207 pattern","Federation Auth Envelope"),HarvestMode.REUSE_VERIFIED,"frontier_resilience_interop_v1.oauth_binding_gate + SOVARA/FIO auth envelope","issuer and client application type are validated before code/token redemption; localhost/native redirects remain explicit and scoped"),
)
