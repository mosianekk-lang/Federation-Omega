...
"""Cognitive Policy Market + Counterfactual Twin + SOL 6.2 constitutional bridge v1.

Dependency is intentionally asymmetric:
- PRIME/CFBE may observe SOL and propose policy.
- SOL never imports this module and remains independently operable.
- This module never dispatches, consumes authority, or performs provider effects.
- Default successful admission is ADVISORY only.
- Bounded topology-control candidacy requires the existing PRIME hosted-shadow +
  Value Foundry bridge to be independently satisfied on the same source head.
"""
...
class PolicyControlScope(str, Enum):
    NONE = "NONE"
    ADVISORY = "ADVISORY"
...
class PolicyProposal:
    ...
    def validate(self) -> "PolicyProposal":
        ...
        for name in (
            "expected_quality", "confidence", "evidence_strength", "reliability",
            "reversibility", "information_gain", "failure_domain_diversity",
            "latency_cost", "monetary_cost", "owner_burden", "risk",
        ):
            value = float(getattr(self, name))
            if not 0.0 <= value <= 1.0:
...
class SolConstitutionalAdmission:
    ...
    control_scope: PolicyControlScope
    advisory_policy_admitted: bool
...
def proposal_from_strategy(
    *, observation: SolObservationEnvelope, decision: PrimeDecisionIR,
    strategy: StrategyCandidate, proposer: PolicySource = PolicySource.BCO_PRIME,
) -> PolicyProposal:
    ...
    return PolicyProposal(
        ...
        confidence=round(
            0.5 * float(strategy.reliability) + 0.5 * float(strategy.evidence_strength), 9
        ),
    ...
    *, observation: SolObservationEnvelope, decision: PrimeDecisionIR,
    ...
    return tuple(
        proposal_from_strategy(
            observation=observation, decision=decision, strategy=by_id[strategy_id]
        )
...
def evaluate_counterfactual(
    proposal: PolicyProposal, observation: SolObservationEnvelope,
    *, scenarios: Sequence[CounterfactualScenario] | None = None,
    ...
    for scenario in cases:
        ...
        resource_penalty = min(
            1.0,
            observation.resource_pressure * scenario.resource_multiplier
            * (0.55 * proposal.latency_cost + 0.45 * proposal.owner_burden),
        )
        contradiction_penalty = min(
            1.0,
            observation.contradiction_pressure * scenario.contradiction_multiplier
        ...
        score = (
            proposal.base_utility() + 0.10 * evidence
            - 0.12 * resource_penalty - 0.15 * contradiction_penalty
            - provider_penalty - assumption_penalty
        )
    ...
    reasons.add(
        ...
        if failures == 0 else "COUNTERFACTUAL_SCENARIO_FAILURES_PRESENT"
    )
    return CounterfactualResult(
        proposal.proposal_id, tuple(scores), mean_score, worst_case,
        failures, robust_score, tuple(sorted(reasons)),
    )


def run_policy_market(
    *, observation: SolObservationEnvelope, proposals: Sequence[PolicyProposal],
    scenarios: Sequence[CounterfactualScenario] | None = None,
    ...
    ranked = sorted(
        validated,
        key=lambda item: (
            -results[item.proposal_id].robust_score,
            -item.base_utility(),
            item.proposal_id,
        ),
    )
    ...
    fallback = next(
        (
            item.proposal_id for item in ranked[1:]
    ...
    reasons.add(
        "PROPOSER_DIVERSITY_SATISFIED" if diversity_satisfied
        else "PROPOSER_DIVERSITY_MISSING"
    )
    draft = {
        "schema": SCHEMA,
        "mission_id": observation.mission_id,
        ...
        "robust_scores": tuple(
            (item.proposal_id, results[item.proposal_id].robust_score) for item in ranked
        ),
...
def _prime_control_qualification_valid(
    qualification: Mapping[str, Any] | None,
    observation: SolObservationEnvelope,
) -> tuple[bool, tuple[str, ...]]:
    if not qualification:
        return False, ("PRIME_VALUE_BRIDGE_EVIDENCE_ABSENT",)
    q = dict(qualification)
    reasons: list[str] = []
    source = str(q.get("source_head_sha") or "").lower()
    if q.get("schema") != PRIME_VALUE_BRIDGE_SCHEMA:
        reasons.append("PRIME_VALUE_BRIDGE_SCHEMA_INVALID")
    if not _valid_git_sha(source) or source != observation.source_version.lower():
        reasons.append("PRIME_VALUE_BRIDGE_SOURCE_MISMATCH")
    if q.get("shadow_evidence_valid") is not True:
        reasons.append("PRIME_SHADOW_EVIDENCE_UNRESOLVED")
    if q.get("rollback_evidence_valid") is not True:
        reasons.append("PRIME_ROLLBACK_EVIDENCE_UNRESOLVED")
    if int(q.get("shadow_pair_count") or 0) < MIN_CONTROL_SHADOW_PAIRS:
        reasons.append("PRIME_THIRTY_SHADOW_PAIRS_REQUIRED")
    if int(q.get("owner_value_pair_count") or 0) < MIN_CONTROL_OWNER_VALUE_PAIRS:
        reasons.append("PRIME_THIRTY_OWNER_VALUE_PAIRS_REQUIRED")
    if q.get("owner_value_proven") is not True:
        reasons.append("PRIME_OWNER_VALUE_NOT_PROVEN")
    if q.get("bounded_topology_control_candidate") is not True:
        reasons.append("PRIME_BOUNDED_CONTROL_NOT_CANDIDATE")
    if q.get("external_effect_control_allowed") is not False:
        reasons.append("PRIME_EXTERNAL_EFFECT_BOUNDARY_VIOLATION")
    if q.get("stable_self_promotion_allowed") is not False:
        reasons.append("PRIME_STABLE_PROMOTION_BOUNDARY_VIOLATION")
    if not str(q.get("receipt_sha256") or ""):
        reasons.append("PRIME_VALUE_BRIDGE_RECEIPT_REQUIRED")
    return not reasons, tuple(sorted(reasons))


def constitutional_admission(
    *, observation: SolObservationEnvelope, market: PolicyMarketDecision,
    proposals: Sequence[PolicyProposal], prime_decision: PrimeDecisionIR,
    prime_control_qualification: Mapping[str, Any] | None = None,
    ...
    advisory = verdict in {AdmissionVerdict.ACCEPT, AdmissionVerdict.CONSTRAIN}
    qualification_valid, qualification_reasons = _prime_control_qualification_valid(
        prime_control_qualification, observation
    )
    bounded_candidate = bool(advisory and qualification_valid)
    if advisory and not bounded_candidate:
        reasons.extend(qualification_reasons)
        reasons.append("ADVISORY_ONLY_UNTIL_PRIME_VALUE_BRIDGE_PASSES")
    if bounded_candidate:
        reasons.append("PRIME_VALUE_BRIDGE_SUPPORTS_BOUNDED_TOPOLOGY_CONTROL_CANDIDATE")
    if bounded_candidate:
        scope = PolicyControlScope.CANDIDATE_BOUNDED_TOPOLOGY_CONTROL
    elif advisory:
        scope = PolicyControlScope.ADVISORY
    else:
        scope = PolicyControlScope.NONE
    ...
    if bounded_candidate:
        proofs.add("BCO_PRIME_VALUE_BRIDGE_RECEIPT")

    draft = {
        "schema": SCHEMA,
        "mission_id": observation.mission_id,
        "verdict": verdict.value,
        "admitted_proposal_id": champion.proposal_id if advisory else None,
        "control_scope": scope.value,
        "advisory_policy_admitted": advisory,
        "bounded_topology_control_candidate": bounded_candidate,
...
def capture_sol_observation(
    runtime: Sol62Runtime, mission_id: str, *,
    provider_runtime_available: bool, exact_authority_available: bool,
    owner_approval_required: bool, evidence_coverage: float, uncertainty: float,
    contradiction_pressure: float, resource_pressure: float, owner_burden: float,
    active_streams: int, source_version: str, proof_refs: Sequence[str] = (),
    ...
    transitions = {
        row["key"]: row["value"] for row in runtime._rows("sol62.transition")
...
def compile_cognitive_policy_cycle(
    *, observation: SolObservationEnvelope, prime_decision: PrimeDecisionIR,
    ...
    prime_proposals = proposals_from_prime(
        observation=observation, decision=prime_decision, strategies=prime_strategies
    )
    proposals = tuple(prime_proposals) + tuple(independent_proposals)
    market = run_policy_market(
        observation=observation, proposals=proposals, scenarios=scenarios
    )
    admission = constitutional_admission(
        ...
        prime_control_qualification=prime_control_qualification,
    )
    qualification_sha = (
        _digest(dict(prime_control_qualification)) if prime_control_qualification else None
    )
    truth_boundary = (
        ...
        "accepted_policy_is_advisory_by_default",
        "bounded_topology_control_is_candidate_only_and_requires_current_source_prime_value_bridge",
        "constitutional_admission_is_not_effect_authority",
        ...
        "stable_self_promotion_remains_prohibited",
...
def cognitive_policy_market_manifest() -> Mapping[str, object]:
    return {
        ...
        "depends_on": (
            "BCO_PRIME_META_EXECUTIVE_V1",
            "BCO_PRIME_SHADOW_VALUE_BRIDGE_V1",
            "CFBE",
            "SOL62",
        ),
        "sol62_imports_policy_market": False,
        "policy_market_may_observe_sol62": True,
        "default_success_scope": PolicyControlScope.ADVISORY.value,
        "new_schedulers": 0,
        "new_memory_roots": 0,
