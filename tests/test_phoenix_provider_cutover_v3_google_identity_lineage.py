...
class GoogleIdentityLineageGuardTests(unittest.TestCase):
    ...
    def test_scripts_run_rejects_distinct_consumer_even_with_other_proof(self):
        result = self.classify(
            ...
            scripts_run_api_enabled=True,
            scripts_run_deployment_verified=True,
            target_authority_verified=True,
            token_issued=True,
            provider_authenticated=True,
            semantic_readback_verified=True,
    ...
    def test_project_management_requires_explicit_apps_script_api_access(self):
        result = self.classify(
            ...
            project_content_inventory_verified=False,
        )
        self.assertEqual(
            result["status"],
            "BLOCKED_ROUTE_PROJECT_RELATIONSHIP",
    ...
    def test_composite_route_requires_both_scripts_run_and_project_management(self):
        result = self.classify(
            route_class=attachment.ROUTE_APPS_SCRIPT_ADMIN_COMPOSITE,
            oauth_consumer_project_number="257649435135",
            transport_project_number=None,
            consumer_identity_verified=True,
            consumer_api_enabled=True,
        ...
        self.assertTrue(result["route"]["scripts_run_relationship_ready"])
        self.assertFalse(
            result["route"]["project_management_relationship_ready"]
        )
        self.assertFalse(result["route"]["relationship_ready"])

    def test_composite_route_can_be_verified_but_never_authorizes_mutation(self):
        result = self.classify(
            route_class=attachment.ROUTE_APPS_SCRIPT_ADMIN_COMPOSITE,
            oauth_consumer_project_number="257649435135",
            transport_project_number=None,
            ...
            scripts_run_api_enabled=True,
            scripts_run_deployment_verified=True,
            project_content_inventory_verified=True,
            target_authority_verified=True,
            token_issued=True,
            provider_authenticated=True,
            semantic_readback_verified=True,
        ...
        self.assertFalse(
            result["invariants"]["classifier_authorizes_provider_mutation"]
        )
