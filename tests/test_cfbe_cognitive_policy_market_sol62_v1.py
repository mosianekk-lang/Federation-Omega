...
def qualifying_value_bridge() -> dict:
    return {
        "schema": "BCO-PRIME-VALUE-BRIDGE-V1",
        "source_head_sha": SOURCE_SHA,
        "shadow_evidence_valid": True,
        "rollback_evidence_valid": True,
        "shadow_pair_count": 30,
        "owner_value_pair_count": 30,
        "shadow_quality_delta": 0.10,
        "owner_value_proven": True,
        "bounded_topology_control_candidate": True,
        "external_effect_control_allowed": False,
        "stable_self_promotion_allowed": False,
        "decision": "CANDIDATE_BOUNDED_TOPOLOGY_CONTROL",
        "blockers": (),
        "truth_boundary": (),
...
class CognitivePolicyMarketSol62Tests(unittest.TestCase):
    ...
    def test_default_success_is_advisory_not_bounded_control_or_dispatch(self) -> None:
        receipt = compile_cognitive_policy_cycle(
            observation=self.observe(),
            prime_decision=self.decision(),
            prime_strategies=self.strategies,
        )
        self.assertEqual(AdmissionVerdict.ACCEPT, receipt.admission.verdict)
        self.assertTrue(receipt.admission.advisory_policy_admitted)
        self.assertEqual(PolicyControlScope.ADVISORY, receipt.admission.control_scope)
        self.assertFalse(receipt.admission.bounded_topology_control_candidate)
        self.assertFalse(receipt.admission.dispatch_authorized)
        self.assertFalse(receipt.admission.external_effect_authorized)
        self.assertFalse(receipt.admission.stable_self_promotion_allowed)

    def test_current_source_value_bridge_can_raise_scope_only_to_bounded_candidate(self) -> None:
        receipt = compile_cognitive_policy_cycle(
            observation=self.observe(),
            prime_decision=self.decision(),
            prime_strategies=self.strategies,
            prime_control_qualification=qualifying_value_bridge(),
        )
        self.assertEqual(AdmissionVerdict.ACCEPT, receipt.admission.verdict)
        self.assertEqual(
            PolicyControlScope.CANDIDATE_BOUNDED_TOPOLOGY_CONTROL,
            receipt.admission.control_scope,
        )
        self.assertTrue(receipt.admission.bounded_topology_control_candidate)
        self.assertFalse(receipt.admission.dispatch_authorized)
        self.assertFalse(receipt.admission.external_effect_authorized)
        self.assertFalse(receipt.admission.stable_self_promotion_allowed)

    def test_stale_value_bridge_cannot_promote_control(self) -> None:
        stale = dict(qualifying_value_bridge())
        stale["source_head_sha"] = "c" * 40
        receipt = compile_cognitive_policy_cycle(
            observation=self.observe(),
            prime_decision=self.decision(),
            prime_strategies=self.strategies,
            prime_control_qualification=stale,
        )
        self.assertEqual(PolicyControlScope.ADVISORY, receipt.admission.control_scope)
        self.assertFalse(receipt.admission.bounded_topology_control_candidate)
        self.assertIn("PRIME_VALUE_BRIDGE_SOURCE_MISMATCH", receipt.admission.reason_codes)
    ...
    def test_high_uncertainty_requires_independent_proposer_not_more_prime_routes(self) -> None:
        receipt = compile_cognitive_policy_cycle(
            ...
            prime_decision=self.decision(uncertainty=0.80),
            prime_strategies=self.strategies,
        )
        ...
        self.assertIn(
            "COMMISSION_INDEPENDENT_POLICY_CHALLENGER",
            receipt.admission.required_actions,
        )
    ...
    def test_counterfactual_provider_loss_penalizes_provider_dependent_policy(self) -> None:
        ...
        proposal = proposal_from_strategy(
            observation=observation,
            decision=decision,
            strategy=external,
        )
        ...
        self.assertIn(
            "COUNTERFACTUAL_PROVIDER_LOSS_DEGRADES_POLICY",
            result.reason_codes,
        )

    def test_sol_current_authority_overrides_stale_prime_assumption(self) -> None:
        observation = self.observe(authority=False)
        prime_obs = prime_observation(
        ...
        scientist = replace(
            prime[1],
            proposal_id="scientist:effect-b",
            proposer=PolicySource.OMEGA_SCIENTIST,
        )
        receipt = compile_cognitive_policy_cycle(
            observation=observation,
        ...
        self.assertIn(
            "EXACT_EFFECT_AUTHORITY_UNAVAILABLE",
    ...
    def test_provider_runtime_gap_is_scoped_defer_not_global_failure(self) -> None:
        ...
        self.assertEqual(PolicyControlScope.NONE, receipt.admission.control_scope)
    ...
    def test_high_contradiction_with_independent_challenger_is_constrained(self) -> None:
        ...
        challenger = replace(
            prime[1],
            proposal_id="adversarial:route-b",
            proposer=PolicySource.ADVERSARIAL_TWIN,
        )
        receipt = compile_cognitive_policy_cycle(
            observation=observation,
            prime_decision=decision,
        ...
        self.assertIn(
            "CONTRADICTION_RESOLUTION_PROOF",
            receipt.admission.proof_requirements,
        )
        self.assertIn(
            "RUN_ADVERSARIAL_VALIDATION_BEFORE_EXECUTION",
            receipt.admission.required_actions,
        )

    def test_owner_gate_stays_owner_gate(self) -> None:
        receipt = compile_cognitive_policy_cycle(
    ...
    def test_verified_reality_mission_rejects_new_meta_policy(self) -> None:
        ...
        self.assertIn(
            "MISSION_ALREADY_VERIFIED_REALITY",
            receipt.admission.reason_codes,
        )
    ...
    def test_objective_drift_fails_before_market(self) -> None:
        observation = replace(
            self.observe(),
            objective_sha256=sha("different objective"),
        )
    ...
    def test_manifest_adds_no_scheduler_memory_executor_or_authority_plane(self) -> None:
        ...
        self.assertEqual("ADVISORY", manifest["default_success_scope"])
