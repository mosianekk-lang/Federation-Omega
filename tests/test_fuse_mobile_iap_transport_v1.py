...
class FuseMobileIapTransportV1Tests(unittest.TestCase):
    ...
    def test_google_signin_dependency_is_exact_and_public_config_only(self) -> None:
        ...
        self.assertIn("EXPO_PUBLIC_FUSE_IAP_CLIENT_ID", env)
    ...
    def test_google_identity_token_is_ephemeral_not_securestore_state(self) -> None:
        iap = (MOBILE / "src" / "iap.ts").read_text()
        session = (MOBILE / "src" / "session.ts").read_text()
        self.assertIn("EXPO_PUBLIC_FUSE_IAP_CLIENT_ID", iap)
