...
def source(source_id: str, *, parent: str | None = None, primary: bool = True) -> SourceRecord:
    return SourceRecord(
        source_id=source_id,
        provenance_class=(ProvenanceClass.PRIMARY_EVIDENCE if primary else ProvenanceClass.DERIVATIVE_SUMMARY),
...
class JfrieV2CoreParityTests(unittest.TestCase):
    ...
    def test_derivative_copies_do_not_multiply_independent_support(self) -> None:
        self.graph.register_source(source("SRC-COPY-1", parent="SRC-PRIMARY-1", primary=False))
        self.graph.register_source(source("SRC-COPY-2", parent="SRC-PRIMARY-1", primary=False))
        roots = self.graph.independent_source_roots(("SRC-PRIMARY-1", "SRC-COPY-1", "SRC-COPY-2"))
    ...
    def test_claim_revision_preserves_mutation_history(self) -> None:
        ...
        self.assertEqual("Exact proposition CLM-1", self.graph.mutations[0].prior_text)

    def test_quarantine_calculates_radius_and_recall_without_destroying_unaffected_content(self) -> None:
        self.graph.register_claim(claim("CLM-ROOT"))
        self.graph.register_claim(claim("CLM-CHILD", dependencies=("CLM-ROOT",)))
        ...
        result = self.engine.invalidate_and_recall("CLM-ROOT", reason="material contradiction", timestamp="2026-08-12T02:00:00Z")
        self.assertEqual(("CLM-CHILD", "CLM-ROOT"), result.contamination_radius)
        self.assertEqual(("ART-A",), result.affected_artifacts)
        self.assertTrue(result.recall_required)
        self.assertEqual(ContaminationState.QUARANTINED, self.graph.claims["CLM-ROOT"].contamination_state)
        self.assertEqual(ContaminationState.NEEDS_REVIEW, self.graph.claims["CLM-CHILD"].contamination_state)
        self.assertEqual(ContaminationState.CLEAN, self.graph.claims["CLM-UNRELATED"].contamination_state)

    def test_synchronization_claim_requires_readback(self) -> None:
        self.assertFalse(self.graph.synchronization_verified(""))
        self.assertTrue(self.graph.synchronization_verified("provider-readback-001"))

    def test_v1_hard_gate_is_preserved_inside_v2_release(self) -> None:
        self.graph.register_claim(claim("CLM-1"))
        invalid = valid_referral()
        invalid.cause_authority_ref = None
        decision = self.engine.evaluate_release(ReleaseRequest(
            referral=invalid,
            claim_ids=("CLM-1",),
    ...
    def test_release_firewall_requires_gates_readback_snapshot_and_recheck(self) -> None:
        ...
        self.assertIn("VERSION_IDENTIFIABLE_RELEASE_SNAPSHOT_REQUIRED", decision.blockers)

    def test_quarantined_or_contradicted_claim_blocks_release(self) -> None:
        ...
        self.assertFalse(decision.allowed)
        self.assertTrue(any(item.startswith("CLAIM_NOT_RELEASE_VERIFIED:CLM-Q") for item in decision.blockers))
        self.assertTrue(any(item.startswith("CLAIM_CONTAMINATED:CLM-Q") for item in decision.blockers))
    ...
    def test_clean_release_requires_all_v1_and_v2_controls(self) -> None:
        ...
        decision = self.engine.evaluate_release(ReleaseRequest(
            referral=valid_referral(),
            claim_ids=("CLM-1",),
            mandatory_gates={"truthgrid": True, "lex": True, "caseforge": True},
            owner_exclusions_passed=True,
            jfrie_recheck_passed=True,
