...
class AOCraFederationInheritanceTests(unittest.TestCase):
    def test_policy_covers_all_registered_federation_engine_families(self) -> None:
        ...
        self.assertIn("BUILD-AO-009", policy["canonical_build_registry"]["current_builds"])
    ...
    def test_bootstrap_preserves_awareness_and_requires_ao_cra(self) -> None:
        bootstrap = json.loads(BOOTSTRAP.read_text(encoding="utf-8"))
        self.assertTrue(bootstrap["surface_awareness"]["required"])
        self.assertIn("FEDOMEGA-SURFACE-AWARENESS-V1", bootstrap["inherited_policies"])
        ...
        self.assertIn("AO_CRA_CREATE_OR_UPDATE_BUILD_FOR_EVERY_RESIDUAL_CAPABILITY_GAP", bootstrap["n_directive"]["required_sequence"])
        self.assertTrue(bootstrap["ao_cra"]["mandatory_for_all_engines"])
        self.assertTrue(bootstrap["ao_cra"]["mandatory_for_all_operations"])
