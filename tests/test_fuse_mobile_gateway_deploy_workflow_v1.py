...
class FuseMobileGatewayDeployWorkflowV1Tests(unittest.TestCase):
    ...
    def test_secretless_firestore_session_dependency_is_fail_closed(self) -> None:
        ...
        self.assertIn("--remove-secrets FUSE_MOBILE_SESSION_SECRET_B64", text)
        self.assertIn("'FUSE_MOBILE_SESSION_SECRET_B64' not in env_names", text)
        self.assertEqual(text.count("FUSE_MOBILE_SESSION_SECRET_B64"), 2)
        self.assertIn("legacy_secret_binding_removal", text)
        self.assertIn("legacy_session_secret_binding_present':False", text)
        self.assertNotIn("SESSION_SECRET_ID", text)
        self.assertNotIn("--set-secrets", text)
        self.assertNotIn("--clear-secrets", text)
