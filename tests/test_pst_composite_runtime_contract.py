...
LOCAL_BIBLE_BOUNDARY = ROOT / "phoenix" / "LOCAL_BIBLE_RECOVERY_EXTERNALIZATION.md"
...
class PstCompositeRuntimeContractTests(unittest.TestCase):
    ...
    def test_local_bible_recovery_is_externalized_from_legacy_source_plane(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        forbidden = (
            "[BIBLE-REBUILD]",
            "'phoenix-local-bible-rebuild'",
            "id: bible_rebuild",
        ...
        boundary = LOCAL_BIBLE_BOUNDARY.read_text(encoding="utf-8")
        ...
        self.assertIn("`[BIBLE-REBUILD]` is not an active source-repository trigger", boundary)

    def test_passive_runs_require_no_pst_completion(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        ...
        self.assertIn("Phoenix quarantine and exports verified; PST not requested", text)
