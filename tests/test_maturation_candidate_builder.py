...
HEAD = "a2f9f3f8afc02205a0ca040963a78466d93e076c"
NOW = "2026-08-30T23:00:00+00:00"
...
def request(**changes: object) -> CandidateBuildRequest:
    values: dict[str, object] = {
        ...
        "head_sha": HEAD,
        "base_ref": "main",
        "target_branch": "codex/cfbe-bubbles-cognitive-court-v1",
        "observed_at": NOW,
...
class CandidateBuilderTests(unittest.TestCase):
    def test_builds_five_observed_provider_disabled_pairs(self) -> None:
        receipt = SuperiorLogicCandidateBuilder().build(request())
        self.assertEqual("CANDIDATE_CANARY_ASSURED", receipt.status)
        self.assertEqual(5, len(receipt.observations))
        self.assertTrue(all(item.execution_observed for item in receipt.observations))
        self.assertTrue(all(not item.provider_execution for item in receipt.observations))
        self.assertTrue(all(not item.external_effect for item in receipt.observations))

    def test_integrates_bubbles_court_without_effect_authority(self) -> None:
        receipt = SuperiorLogicCandidateBuilder().build(request())
        self.assertTrue(all(item.court_state == "SELECTED_NO_EFFECT" for item in receipt.observations))
        self.assertTrue(all(not item.effect_authorized for item in receipt.observations))
        self.assertTrue(all(len(item.court_receipt_sha256) == 64 for item in receipt.observations))
    ...
    def test_independent_assurance_passes_but_holds_stable_promotion(self) -> None:
        ...
        self.assertEqual("HOLD_FOR_30_PAIR_GATE", receipt.assurance.decision)
    ...
    def test_all_expected_challengers_are_selected_and_quality_is_protected(self) -> None:
        observations = SuperiorLogicCandidateBuilder().build(request()).observations
        self.assertTrue(all(item.expectation_met for item in observations))
        self.assertTrue(all(item.quality_protected for item in observations))
    ...
    def test_assurance_detects_manifest_hash_tampering(self) -> None:
        ...
        assurance = IndependentAssuranceCourt().assure(manifest=tampered, observations=receipt.observations)
        self.assertFalse(assurance.assurance_passed)
        self.assertEqual("REJECT", assurance.decision)

    def test_assurance_detects_observation_tampering(self) -> None:
        receipt = SuperiorLogicCandidateBuilder().build(request())
        changed = replace(receipt.observations[0], provider_execution=True)
        assurance = IndependentAssuranceCourt().assure(
            manifest=receipt.candidate_manifest,
            observations=(changed,) + receipt.observations[1:],
        )
        self.assertFalse(assurance.assurance_passed)
    ...
    def test_write_receipts_persists_five_hash_bound_artifacts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            observations = json.loads((Path(temp_dir) / "observed_challenger_receipts.json").read_text())
    ...
    def test_cli_executes_from_repository_root(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            command = [
                ...
                "--base-ref", "main", "--target-branch", "codex/cfbe-bubbles-cognitive-court-v1",
                "--observed-at", NOW,
            ]
            ...
            self.assertEqual("CANDIDATE_CANARY_ASSURED", payload["status"])
