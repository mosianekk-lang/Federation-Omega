...
def test_mission_spec_can_reuse_legacy_contract_identity():
    legacy = {
    ...
    assert spec.mission_id == legacy["mission_id"]
    assert spec.source_contract_sha256 == "abc123"
    assert spec.source_contract_ref == "FEDERATION_OMEGA_V2_MISSION_CONTRACT"
    assert spec.mission_sha256


def test_public_safe_mission_rejects_secret_fields_via_ledger():
    ledger = ConvergenceLedger()
    with pytest.raises(ValueError, match="secret-bearing"):
        ledger.append(mission_id="MISSION-X", event_type="TEST", payload={"api_key": "do-not-store"})


def test_jsonl_ledger_is_idempotent_and_restart_verifiable(tmp_path: Path):
    path = tmp_path / "mce.jsonl"
    ...
    assert one.event_id == replay.event_id
    assert len(ledger.events()) == 1
    reopened = ConvergenceLedger(path)
    result = reopened.verify()
    assert result["state"] == "VERIFIED"
    assert result["event_count"] == 1
    assert result["head_hash"] == one.event_hash


def test_dependency_wave_runs_independent_lanes_and_serializes_shared_state():
    ...
    assert [item.work_id for item in first] == ["A"]
    ...
    assert "D" in ids
    assert len({"B", "C"} & ids) == 1


def test_closure_lock_only_p0_interrupts_implementation():
    lock = ClosureLock(active=True, target="Close live canary")
    assert lock.disposition(IdeaPriority.P0) == IdeaDisposition.INTERRUPT_CLOSURE
    assert lock.disposition(IdeaPriority.P1) == IdeaDisposition.PARALLEL_CHALLENGER
    assert lock.disposition(IdeaPriority.P2) == IdeaDisposition.IMPROVEMENT_INBOX
    assert lock.disposition(IdeaPriority.P3) == IdeaDisposition.FUTURE_BACKLOG


def test_failure_fingerprint_reuses_one_resolver_record():
    engine = MissionConvergenceEngine()
    engine.open_mission(mission())
    ...
    assert len(projection.resolvers) == 1
    assert projection.resolvers[resolver.resolver_id].occurrence_count == 2


def test_closure_requires_success_proof_axes_work_and_closed_failures():
    engine = MissionConvergenceEngine()
    engine.open_mission(mission())
    ...
    engine.record_failure("MISSION-MCE-TEST-001", resolver)
    assert not engine.project("MISSION-MCE-TEST-001").closable
    ...
    assert engine.project("MISSION-MCE-TEST-001").closable
    receipt = engine.close_mission("MISSION-MCE-TEST-001", receipt_refs=("JARVIS-READBACK",))
    assert receipt["state"] == "CLOSED_VERIFIED"
    assert receipt["closure_receipt_sha256"]
    assert engine.project("MISSION-MCE-TEST-001").status == "CLOSED_VERIFIED"


def test_proven_axis_requires_evidence():
    with pytest.raises(ValueError, match="requires evidence"):
        ProofEntry.create(axis="source", status=ProofStatus.PROVEN)


def test_event_projection_preserves_append_only_history():
    engine = MissionConvergenceEngine()
    engine.open_mission(mission())
    ...
    assert projection.proof_vector["source"].status == ProofStatus.PROVEN
    assert len(engine.ledger.events("MISSION-MCE-TEST-001")) >= 4
    assert engine.ledger.verify()["state"] == "VERIFIED"
