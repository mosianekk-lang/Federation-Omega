...
from omega_one.hyperperformance import ExactlyOnceFinalizer, FinalizationDecision, MissionMeasurement, OutcomeState, PairedMissionObservation, evaluate_paired_campaign
...
class OmegaOneCurrentReanchorTests(unittest.TestCase):
    ...
    def test_external_effect_contract_remains_held_without_sovara_authority(self):
        contract = UniversalCapabilityContract(capability_id="CAP-SEND", name="Send Message", description="Bounded effect test", input_schema={"type":"object"}, output_schema={"type":"object"}, effect_class=EffectClass.EXTERNAL_EFFECT, rollback_required=True)
    ...
    def test_exactly_once_replay_keeps_one_canonical_receipt(self):
        ...
        first = finalizer.finalize("op-1", {"x":1}, {"ok":True}, {"proof":"p"}, OutcomeState.SUCCEEDED)
        replay = finalizer.finalize("op-1", {"x":1}, {"ok":True}, {"proof":"p"}, OutcomeState.SUCCEEDED)
    ...
    def test_paired_campaign_can_only_qualify_local_non_provider_measurement(self):
        pairs = tuple(PairedMissionObservation(MissionMeasurement(f"m-{i}", f"sha256:oracle-{i}", 10.0, 1.0), MissionMeasurement(f"m-{i}", f"sha256:oracle-{i}", 5.0, 1.0)) for i in range(30))
    ...
    def test_current_proofos_selects_omega_one_without_full_fallback(self):
        ...
        impact = ImpactCompiler(policy).assess(["omega_one/hyperperformance.py", "benchmarking/omega_one_cfbe_local.py", "tests/test_omega_one_current_reanchor_v1.py"])
        manifest = ProofSelector(policy).compile_manifest(base_sha="a"*40, head_sha="b"*40, impact=impact)
