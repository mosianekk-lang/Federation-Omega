...
class ForestFirstArchitectureInterfaceV1Tests(unittest.TestCase):
    ...
    def test_top_level_admission_requires_all_four_criteria(self):
        denied = self.registry.admit_top_level_system(
            unique_authority=True,
            unique_state=True,
            unique_runtime=True,
            unique_failure_domain=False,
        )
        self.assertFalse(denied.admitted_as_top_level_system)
        self.assertEqual(denied.missing_criteria, ("UNIQUE_FAILURE_DOMAIN",))
        admitted = self.registry.admit_top_level_system(
            unique_authority=True,
            unique_state=True,
            unique_runtime=True,
            unique_failure_domain=True,
        )
    ...
    def test_forbidden_transitions_are_machine_checkable(self):
        self.assertTrue(
            self.registry.transition_forbidden(
                "ASSURANCE_REALITY", "MISSION_EXECUTION", "EXECUTE_EXTERNAL_EFFECT"
            )
        )
        self.assertTrue(
            self.registry.transition_forbidden(
                "LEARNING_EVOLUTION", "DOMAIN_INTELLIGENCE", "INHERIT_PROVIDER_OR_DOMAIN_AUTHORITY"
            )
        )
        self.assertTrue(
            self.registry.transition_forbidden(
                "MISSION_EXECUTION", "HUMAN_SOVEREIGNTY", "SILENTLY_OVERRIDE_RESERVED_CONSEQUENTIAL_DECISION"
            )
        )

    def test_independent_assurance_is_preserved(self):
        self.assertEqual(
            set(self.registry.independent_systems()),
