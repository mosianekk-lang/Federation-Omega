...
class RuntimeAttestationTests(unittest.TestCase):
    ...
    def test_current_chat_does_not_preprove_cross_chat_stage16(self) -> None:
        attestation = self.attestation(activation_kind=ActivationKind.CURRENT_CHAT)
...
class AutonomousRegressionPlannerTests(unittest.TestCase):
    def test_real_failure_memory_becomes_regression_contract(self) -> None:
        entry = FailureMemoryEntry(
            fingerprint="STALE_BASE_HEAD_REJECTED",
            repair_action="RECUT_CURRENT_MAIN",
            repair_proof_ref="PR334",
        )
        case = AutonomousRegressionPlanner().from_failure(entry)
        self.assertIn("recut from current main", case.expected_behavior)
        self.assertIn("weaken ancestry", case.prohibited_behavior)
        self.assertTrue(case.rollback_required)
...
class CapabilityFormationTests(unittest.TestCase):
    def test_source_only_twin_generates_governed_capability_build(self) -> None:
        ...
        twin = source_twin("KAIO")
        gap = engine.infer_gap(twin, required_role="PROVIDER_BOUND_MODEL_RUNTIME")
    ...
    def test_provider_verified_twin_does_not_invent_gap(self) -> None:
        self.assertIsNone(
            CapabilityFormationEngine().infer_gap(
                provider_twin(),
                required_role="TESTED_GOOGLE_SHEETS_PROVIDER_ROUTE",
            )
        )


class DominanceControllerTests(unittest.TestCase):
    def signal(self, system_id: str, priority_seed: float) -> WeaknessSignal:
        return WeaknessSignal(
            system_id=system_id,
            materiality=priority_seed,
            recurrence=priority_seed,
            downstream_impact=priority_seed,
            proof_gap=priority_seed,
            owner_burden=priority_seed,
            proof_ref=f"RCP:{system_id}",
        )

    def full_maturity_proof(self, *, include_provider: bool) -> MaturityProofEnvelope:
        return MaturityProofEnvelope(
            ...
            provider_readback_ref="R-PROVIDER" if include_provider else "",
    ...
    def test_controller_cannot_claim_dominance_without_provider_proof(self) -> None:
        decision = AutonomousMaturityDominanceController().decide(
            ...
            maturity_proof=self.full_maturity_proof(include_provider=False),
    ...
    def test_controller_selects_highest_priority_weakness(self) -> None:
        controller = AutonomousMaturityDominanceController()
        chosen = controller.choose_weakness(
            (self.signal("KAIO", 0.4), self.signal("TRUTHGRID", 0.9), self.signal("MODISA", 0.7))
        )
        self.assertEqual("TRUTHGRID", chosen.system_id)

    def test_existing_mission_work_takes_priority_over_new_evolution_experiment(self) -> None:
        decision = AutonomousMaturityDominanceController().decide(
    ...
    def test_full_proof_allows_candidate_label_only_through_strict_gate(self) -> None:
        decision = AutonomousMaturityDominanceController().decide(
            completed_through=EvolutionStage.AUTONOMOUS_MATURITY_DOMINANCE_CONTROLLER,
            maturity_proof=self.full_maturity_proof(include_provider=True),
            weaknesses=(),
