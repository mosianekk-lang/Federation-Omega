...
app = FastAPI(title="AEGIS-Ω", version="0.3.1", description="Defensive adversarial mobile threat-defense control plane")
...
def _ensure_outbox_delivery(case: dict) -> None:
    """Publish with a single active claimant; delivery semantics remain at-least-once.

    A crash after Pub/Sub accepts the message but before the outbox mark can cause a
    later re-publish after the claim expires. The event bus therefore carries the
    deterministic `aegis.assessment:<case_id>` idempotency key for downstream dedup.
    """
    ...
    try:
        claim = store.claim_outbox_delivery(assessment.case_id, claimant, lease_seconds=30.0)
    except Exception as exc:
        raise HTTPException(
            status_code=503,
            detail={"error": "AEGIS_OUTBOX_CLAIM_FAILED", "case_id": assessment.case_id, "retry_safe": True},
        ) from exc

    if claim.get("already_delivered") is True:
        return
    if claim.get("claimed") is not True:
        raise HTTPException(
            status_code=503,
            detail={"error": "AEGIS_OUTBOX_DELIVERY_IN_PROGRESS", "case_id": assessment.case_id, "retry_safe": True},
        )

    try:
        provider_ref = event_bus.publish_assessment(assessment)
        store.mark_outbox_delivered(assessment.case_id, str(provider_ref or ""), claimant_id=claimant)
    except Exception as exc:
        try:
            store.release_outbox_claim(assessment.case_id, claimant)
        except Exception:
            pass
        raise HTTPException(
            status_code=503,
            detail={"error": "AEGIS_OUTBOX_DELIVERY_PENDING", "case_id": assessment.case_id, "retry_safe": True},
        ) from exc
...
def healthz():
    return {
        ...
        "env": settings.env,
        "mode": "defensive-only",
        "adversarial_mode": "synthetic-defensive-only",
        "delivery_semantics": "AT_LEAST_ONCE_WITH_DETERMINISTIC_DEDUP",
    }
...
def ingest_event(event: SecurityEvent):
    ...
    try:
        existing = store.resolve_request(request_id, request_hash)
    except IdempotencyCollision as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    if existing:
        _ensure_outbox_delivery(existing)
        return _assessment_from_case(existing)
    ...
    try:
        stored = store.put(assessment, [sealed], request_id=request_id, request_hash=request_hash)
    except IdempotencyCollision as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    _ensure_outbox_delivery(stored)
    return _assessment_from_case(stored)
...
def assess(req: AssessmentRequest):
    ...
    try:
        normalized = [normalize_event(e) for e in req.events]
    except ValueError as exc:
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    try:
        existing = store.resolve_request(req.request_id, request_hash)
    except IdempotencyCollision as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    if existing:
        _ensure_outbox_delivery(existing)
    ...
    assessment = assess_events(normalized, settings.manual_approval_threshold, settings.high_risk_threshold)
    for item in sealed:
        evidence_store.persist(item)
    try:
        stored = store.put(assessment, sealed, request_id=req.request_id, request_hash=request_hash)
    except IdempotencyCollision as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    _ensure_outbox_delivery(stored)
    return _assessment_from_case(stored)
...
def get_case(case_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="case evidence chain unreadable") from exc
    if not chain_valid:
        raise HTTPException(status_code=500, detail="case evidence chain verification failed")
    plan = plan_response(Assessment.model_validate(case["assessment"]))
    return {
        **case,
        "evidence_chain_valid": True,
        "outbox_delivery": store.outbox_state(case_id),
        "response_plan": {"automated": plan.automated, "approval_required": plan.approval_required},
    }


@app.post("/v1/evolution/certify", response_model=CertificationResult)
def certify_candidate(metrics: CandidateMetrics):
    return certify(metrics)


@app.get("/v1/orchestration/profile")
def orchestration_profile():
    return mission_plan_public_dict(aegis_current_mission_plan())


@app.get("/v1/adversarial/profile")
def adversarial_profile():
    return {
        "schema": "AEGIS_ADVERSARIAL_PROFILE_V1",
        "defensive_only": True,
        "synthetic_scenarios_only": True,
        "components": [
            "OMEGA_ADVERSARY_TWIN",
            "OMEGA_EVIDENCE_IMMUNE_GRAPH",
            "OMEGA_SHADOW_NEURAL_SENTINEL",
            "OMEGA_EVOLUTION_TOURNAMENT",
            "OMEGA_ADVERSARIAL_COURT",
        ],
        "neural_decision_authority": False,
        "automatic_model_promotion": False,
        "provider_effect_authority": False,
        "high_impact_response_requires_human_approval": True,
    }
