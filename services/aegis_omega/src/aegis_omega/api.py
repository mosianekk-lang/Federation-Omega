...
def _ensure_outbox_delivery(case: dict) -> None:
    """Publish at-least-once with a single active publisher claim and deterministic downstream dedupe key."""
    assessment = _assessment_from_case(case)
    state = store.outbox_state(assessment.case_id)
    if state and state.get("delivered") is True:
        return

    claimant = f"api-{uuid.uuid4().hex}"
    try:
        claimed = store.claim_outbox(assessment.case_id, claimant, OUTBOX_CLAIM_TTL_SECONDS)
    ...
    try:
        ...
        store.mark_outbox_delivered(assessment.case_id, str(provider_ref or ""), claimant=claimant)
    except Exception as exc:
        try:
...
def healthz():
    return {
        ...
        "delivery_semantics": "at-least-once-with-deterministic-dedup",
    }
