...
from .relay_protocol import sign_request, signing_payload
...
def _dpapi(data: bytes, *, protect: bool) -> bytes:
    ...
    crypt32.CryptProtectData.argtypes = [ctypes.POINTER(DATA_BLOB), wintypes.LPCWSTR, ctypes.POINTER(DATA_BLOB), ctypes.c_void_p, ctypes.c_void_p, wintypes.DWORD, ctypes.POINTER(DATA_BLOB)]; crypt32.CryptProtectData.restype = wintypes.BOOL
    crypt32.CryptUnprotectData.argtypes = [ctypes.POINTER(DATA_BLOB), ctypes.POINTER(wintypes.LPWSTR), ctypes.POINTER(DATA_BLOB), ctypes.c_void_p, ctypes.c_void_p, wintypes.DWORD, ctypes.POINTER(DATA_BLOB)]; crypt32.CryptUnprotectData.restype = wintypes.BOOL
    kernel32.LocalFree.argtypes = [ctypes.c_void_p]; kernel32.LocalFree.restype = ctypes.c_void_p
    if protect: ok = crypt32.CryptProtectData(ctypes.byref(source), "FUSE Windows Relay", None, None, None, 1, ctypes.byref(target))
    else:
...
def _b64u(data: bytes) -> str: return base64.urlsafe_b64encode(data).decode("ascii").rstrip("=")
def _unb64u(value: str) -> bytes: return base64.urlsafe_b64decode(value + "=" * (-len(value) % 4))
...
def _save_credential(path: Path, value: dict[str, str]) -> None:
    protected = _dpapi(json.dumps(value, sort_keys=True).encode(), protect=True); path.parent.mkdir(parents=True, exist_ok=True); path.write_text(base64.b64encode(protected).decode("ascii"), encoding="ascii")
...
def _load_credential(path: Path) -> dict[str, str]:
    ...
    if "private_key_pkcs8_b64" not in value: raise RuntimeError("LEGACY_HMAC_CREDENTIAL_REENROLL_REQUIRED")
...
def _signed_headers(credential: dict[str, str], *, method: str, path: str, body: bytes) -> dict[str, str]:
    ...
    if credential.get("device_secret"):
        signature = sign_request(credential["device_secret"], method=method, path=path, timestamp=timestamp, nonce=nonce, body=body)
    else:
...
def enroll(client: requests.Session, *, base_url: str, enrollment_id: str, enrollment_token: str, device_label: str, credential_path: Path) -> dict[str, str]:
    private_b64, public_b64 = _new_software_key(); response = client.post(base_url + "/agent/enroll", json={"enrollment_id": enrollment_id, "enrollment_token": enrollment_token, "device_label": device_label, "public_key_spki_b64": public_b64}, timeout=30); response.raise_for_status(); data = response.json()
    credential = {"device_id": str(data["device_id"]), "private_key_pkcs8_b64": private_b64, "public_key_spki_b64": public_b64, "assurance": "SOFTWARE_DPAPI_LOWER_ASSURANCE"}; _save_credential(credential_path, credential); return credential


def run_once(client: requests.Session, *, base_url: str, workspace: Path, credential: dict[str, str]) -> str:
    poll_path = "/agent/poll"; poll_body = b""; response = client.post(base_url + poll_path, data=poll_body, headers=_signed_headers(credential, method="POST", path=poll_path, body=poll_body), timeout=30); response.raise_for_status(); item = response.json()
    if item.get("state") == "EMPTY": return "EMPTY"
    task = TaskEnvelope.from_mapping(item["task"]); receipt = WindowsPlane(workspace).execute(task).to_dict(); payload = json.dumps({"task_id": task.task_id, "lease_token": item["lease"]["lease_token"], "receipt": receipt}, sort_keys=True, separators=(",", ":")).encode(); complete_path = "/agent/complete"
    completed = client.post(base_url + complete_path, data=payload, headers=_signed_headers(credential, method="POST", path=complete_path, body=payload), timeout=30); completed.raise_for_status(); return "COMPLETED"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="federation-windows-agent"); parser.add_argument("--relay-url", default=os.environ.get("FUSE_RELAY_URL")); parser.add_argument("--workspace", type=Path, default=Path.cwd()); parser.add_argument("--credential-file", type=Path, default=Path(os.environ.get("LOCALAPPDATA", ".")) / "FUSE" / "relay-credential.dpapi"); parser.add_argument("--enrollment-id", default=os.environ.get("FUSE_ENROLLMENT_ID")); parser.add_argument("--enrollment-token", default=os.environ.get("FUSE_ENROLLMENT_TOKEN")); parser.add_argument("--device-label", default=os.environ.get("COMPUTERNAME", "owner-windows")); parser.add_argument("--once", action="store_true"); parser.add_argument("--poll-seconds", type=float, default=5.0); args = parser.parse_args(argv)
