...
_ALLOWED_FIELDS = frozenset({
    "schema", "event_id", "source_class", "event_class", "fingerprint_sha256",
    "matter_class", "materiality", "consequence", "uncertainty", "dependency_density",
    "adversarial_complexity", "deadline_risk", "evidence_risk", "owner_only",
    "provider_readback_missing", "route_failure", "objective_exhausted",
    "material_strategy_change", "private_content_included",
})
_SOURCE_CLASSES = frozenset({"GMAIL_METADATA", "DRIVE_METADATA", "FEDERATION_STATE", "PROVIDER_HEALTH", "DEADLINE_SIGNAL", "SYSTEM_EVENT"})
_EVENT_CLASSES = frozenset({"NEW_ITEM", "STATE_CHANGE", "DEADLINE_CHANGE", "EVIDENCE_CHANGE", "PROVIDER_CHANGE", "FAILURE", "RECOVERY", "NO_MATERIAL_CHANGE"})
...
def _score(event: Mapping[str, object], key: str, default: float) -> float:
...
def validate_event(raw: Mapping[str, object]) -> dict[str, object]:
    ...
    if unknown:
        raise BackgroundEventError("Sanitized event contains unsupported fields: " + ", ".join(unknown))
    if raw.get("schema") != EVENT_SCHEMA:
        raise BackgroundEventError(f"Unsupported background event schema: {raw.get('schema')!r}")
    if _flag(raw, "private_content_included"):
    ...
    return {
        "schema": EVENT_SCHEMA, "event_id": event_id, "source_class": source_class,
        "event_class": event_class, "fingerprint_sha256": fingerprint, "matter_class": matter_class,
        "materiality": _score(raw, "materiality", 0.5), "consequence": _score(raw, "consequence", 0.5),
        "uncertainty": _score(raw, "uncertainty", 0.5), "dependency_density": _score(raw, "dependency_density", 0.5),
        "adversarial_complexity": _score(raw, "adversarial_complexity", 0.5),
        "deadline_risk": _flag(raw, "deadline_risk"), "evidence_risk": _flag(raw, "evidence_risk"),
        "owner_only": _flag(raw, "owner_only"), "provider_readback_missing": _flag(raw, "provider_readback_missing"),
        "route_failure": _flag(raw, "route_failure"), "objective_exhausted": _flag(raw, "objective_exhausted"),
        "material_strategy_change": _flag(raw, "material_strategy_change"), "private_content_included": False,
    }
...
def run_background_event(raw: Mapping[str, object]) -> dict[str, object]:
    ...
    cost = runtime.cost.evaluate(WorkloadCostProfile(
    ...
    high_stakes = bool(event["matter_class"] == "LEGAL" or event["materiality"] >= 0.70 or event["deadline_risk"] or event["evidence_risk"])
    routes = (
        {"route_id": "PRIVATE_REASONING_WAKE", "route_type": "WAKE_PRIVATE_REASONING", "available": True, "authorised": True,
         "feasibility": .95, "proof_strength": .85, "reversibility": 1.0, "speed": .85,
         "strategic_value": .95 if high_stakes else .55, "owner_burden": .20, "privacy_cost": .05,
         "maintenance_cost": .05, "information_gain": .90},
        {"route_id": "DEFER_UNTIL_MATERIAL_DELTA", "route_type": "DEFER", "available": not high_stakes, "authorised": True,
         "feasibility": 1.0, "proof_strength": .70, "reversibility": 1.0, "speed": 1.0,
         "strategic_value": .80 if not high_stakes else .10, "owner_burden": 0.0, "privacy_cost": 0.0,
         "maintenance_cost": 0.0, "information_gain": .20},
    )
    forest = runtime.forest.run(ForestOmegaContext(
        matter_id=f"PRIVATE_{event['matter_class']}_MATTER",
        objective="Classify a sanitized state-change signal without exposing private content",
        desired_outcome="Wake private reasoning only for a material or owner-only signal",
        high_stakes=high_stakes,
        consequence=float(event["consequence"]), uncertainty=float(event["uncertainty"]),
        dependency_density=float(event["dependency_density"]), adversarial_complexity=float(event["adversarial_complexity"]),
        root_hypotheses=("The signal may require private-context recomputation", "The signal may be non-material and should remain quiet"),
        tree_facts=(f"source_class={event['source_class']}", f"event_class={event['event_class']}",
                    f"materiality_band={'HIGH' if event['materiality'] >= .70 else 'LOW_OR_MODERATE'}"),
        ...
        credible_risk_signal_present=bool(event["materiality"] >= .70 or event["deadline_risk"] or event["evidence_risk"]),
        provider_readback_required_but_missing=bool(event["provider_readback_missing"]),
        route_failure_detected=bool(event["route_failure"]), objective_exhausted=bool(event["objective_exhausted"]),
...
__all__ = ["BackgroundEventError", "EVENT_SCHEMA", "RECEIPT_SCHEMA", "run_background_event", "validate_event"]
