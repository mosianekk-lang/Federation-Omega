...
class CareerCapabilityFoundry:
    """Harvest reusable market requirements into proof-bound Bubbles capability growth.

    The foundry develops organisational capability inside the Bubbles engineering
    cell. It must never convert a market requirement or an internal capability pack
    into a claim that the human owner personally possesses that experience.
    """
    ...
    def _proof_gates(capability: CapabilityRequirement) -> tuple[str, ...]:
        base = ["IMPLEMENTED", "TESTED"]
        runtime_domains = {"engineering", "data", "education", "security", "ai"}
        if capability.domain in runtime_domains:
            base.append("RUNTIME_OR_DEMONSTRATION_VERIFIED")
        if "cloud" in capability.required_disciplines or "deployment" in capability.required_disciplines:
            base.append("PROVIDER_READBACK")
        if "pilot" in capability.required_disciplines or capability.domain in {"education", "leadership"}:
            base.append("MEASURED_OUTCOME_OR_PILOT_EVIDENCE")
        base.append("LEDGER_APPROVED_CLAIM")
        return tuple(base)
    ...
    def assess(
        ...
        *,
        ...
        occurrences: dict[str, list[RoleSignal]] = {capability.capability_id: [] for capability in self.capabilities}
        ...
        for capability in self.capabilities:
            ...
            build = len(matched) >= 2 or strategic or capability.strategic_weight >= 3
            state = GrowthState.BUILD if gap != GapClass.COVERED and build else GrowthState.WATCH
            if gap == GapClass.COVERED:
                state = GrowthState.TESTED
            ...
            assessments.append(
                CapabilityAssessment(
                    ...
                    truth_boundary=(
                        "This assessment grows Bubbles organisational capability only. "
                        "It is not evidence that the human owner personally has the role experience; "
                        "personal CV/interview claims require separate owner evidence and Ledger approval."
                    ),
                )
            )
    ...
    def backlog(self, assessments: Iterable[CapabilityAssessment]) -> dict[str, object]:
        ...
        payload = {
            ...
            "items": records,
            "truth_boundary": (
                "Market requirements are learning signals. Bubbles capability growth and Kim personal evidence "
                "are separate proof dimensions; neither may be inferred from the other."
            ),
        }
