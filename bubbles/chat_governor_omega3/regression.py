...
class TraceToRegressionBridge:
    """Durably normalize one observed integrity failure into a replay candidate."""
    ...
    def __init__(
        ...
        state: DurableState,
        *,
        learning_sink: Optional[LearningSink] = None,
    ) -> None:
        self.state = state
    ...
    def capture(self, incident: ObservedIntegrityIncident) -> RegressionCandidate:
        ...
        checkpoint_id = self.state.checkpoint(
            incident.mission_id,
            {
                "event": "CHAT_INTEGRITY_REGRESSION_CANDIDATE",
                "candidate": payload,
        ...
        self.state.update_metric(
            "chatgov.regression.duplicate", 1.0 if existing is not None else 0.0
        )
