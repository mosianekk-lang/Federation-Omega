...
The bridge reuses ChatGov DurableState and can forward a normalized incident to an
existing learning ledger through a callback. It deliberately does not create a
second sovereign learning system and has no provider effects.
"""
...
class ObservedIntegrityIncident:
    ...
    def validate(self) -> None:
        for field in ("mission_id", "failure_code", "claim", "observed_fruit", "desired_outcome"):
...
class TraceToRegressionBridge:
    ...
    def capture(self, incident: ObservedIntegrityIncident) -> RegressionCandidate:
        ...
        self.state.save_receipt(key=key, mission_id=incident.mission_id, action="CAPTURE_CHAT_INTEGRITY_REGRESSION", target=incident.failure_code, success=True, semantic_ok=True, payload=payload)
        checkpoint_id = self.state.checkpoint(incident.mission_id, {"event": "CHAT_INTEGRITY_REGRESSION_CANDIDATE", "candidate": payload, "duplicate": existing is not None}, proof_bearing=False)
        ...
        if self.learning_sink is not None:
            self.learning_sink({"failure_code": incident.failure_code, "claim": incident.claim, "observed_fruit": incident.observed_fruit, "desired_outcome": incident.desired_outcome, "affected_capabilities": list(incident.affected_capabilities), "trace_ref": incident.trace_ref, "reuse_decision": "PATCH_EXISTING"}, (regression_test,))
        return RegressionCandidate(fingerprint=fingerprint, mission_id=incident.mission_id, failure_code=incident.failure_code, regression_test=regression_test, trace_ref=incident.trace_ref, replay_state=replay_state, duplicate=existing is not None, checkpoint_id=checkpoint_id)
