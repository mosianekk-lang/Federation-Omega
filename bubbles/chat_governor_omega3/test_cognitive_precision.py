...
class CognitivePrecisionTests(unittest.TestCase):
        ...
        result = self.kernel.select_next_test([
            {
                ...
                "decision_sensitivity": 0.90,
                "resolution_probability": 0.80,
                "reversibility": 0.80,
                "cost": 0.90,
                "time": 0.80,
                "risk": 0.40,
                "owner_attention": 0.40,
            },
            {
                "experiment_id": "CHEAP",
    ...
    def test_open_high_severity_falsifier_blocks_convergence(self) -> None:
        decision = self.kernel.compile_decision(candidates=[{
            ...
            "independence": 0.90,
            "falsifiers": [{
                "description": "Primary record contradicts X",
                "severity": "HIGH",
                "resolved": False,
            }],
        }])
