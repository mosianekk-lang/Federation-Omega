...
$result = [ordered]@{
  ...
  cpu_inference = $null
  truth = 'WINDOWS_PREFLIGHT_AND_CPU_INFERENCE_ONLY'
}
foreach ($t in $tools) {
...
$required = @('FUSE-AIOS-CONSTITUTION.md','architecture\SOVEREIGNTY-DEPENDENCY-GRAPH.json','architecture\CFBE-CAPABILITY-MATRIX.json','boot\boot-contract.json','ai\smoke-model.json','ai\inference_smoke.py')
