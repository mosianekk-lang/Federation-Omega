        ...
        for item in opportunities:
            ...
            signals.append(
                BottleneckSignal(
                    ...
                    latency_share=leverage if item.kind == "LATENCY_BOTTLENECK" else 0.25 * leverage,
                    queue_wait_share=0.30 * leverage,
                    failure_recurrence=0.25 * leverage,
                    dependency_centrality=0.55 * leverage,
                    owner_burden=0.35 * leverage,
                    cost_pressure=0.25 * leverage,
                    proof_gap=0.45 if item.kind == "CAPABILITY_GAP" else 0.25,
                    risk=0.55 if item.kind == "REVERSIBILITY_OR_RISK_GAP" else 0.25,
