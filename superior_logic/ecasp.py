...
class CorpusPreservationState(str, Enum):
...
class CorpusActivationState(str, Enum):
...
TRIGGER_PHRASES = (
    "full sweep", "exhaustive sweep", "audit everything", "read all", "do all",
    "best combination", "strongest code", "strongest stack", "select the best",
    "complete archive", "entire corpus", "all emails", "all documents", "all modules",
    "final stack", "fully reconcile", "no stone unturned",
)

EXHAUSTIVE_CLAIM_TERMS = (
    "best", "final", "full", "complete", "exhaustive", "strongest", "fully reconciled",
)
...
class CorpusObject:
    ...
    def non_dilution_compliant(self) -> bool:
        if self.permanent_exclusion_requested:
            return bool(self.owner_decision_reference and self.preservation_copy_reference)
        return self.preservation_state in set(CorpusPreservationState)

    def analytical_chain_complete(self) -> bool:
        if self.excluded_as_immaterial:
            return bool(self.exclusion_reason) and self.non_dilution_compliant()
        return all(
            (
                self.accounted_body(), self.parsed, self.attachments_complete(),
                self.module_decomposed, self.deduped, self.version_reconciled,
                self.conflict_tested, self.requirement_coverage_tested,
                self.selected_or_rejected, self.verified, self.non_dilution_compliant(),
            )
        )
...
def evaluate_ecasp(request: ECASPRequest) -> ECASPResult:
    ...
    preserved = sum(1 for item in objects if item.non_dilution_compliant())
    permanent_exclusion_requests = sum(1 for item in objects if item.permanent_exclusion_requested)
    ...
    g6 = request.conflict_dependency_matrix_complete and _all(objects, "deduped") and _all(objects, "conflict_tested")
    g7 = request.requirement_coverage_complete and _all(objects, "requirement_coverage_tested")
    ...
    g11 = bool(objects) and all(item.non_dilution_compliant() for item in objects)
    ...
    gates_before_claim = all((g1, g2, g3, g4, g5, g6, g7, g8, g9, g11))
    ...
    gates = (
        ...
        GateResult("G10_CLAIM_LANGUAGE", g10, "claim strength must not exceed the other release gates"),
        GateResult(
            "G11_NON_DILUTION_PRESERVATION",
            g11,
            f"preserved={preserved}; expected={request.expected_object_count}; permanent_exclusion_requests={permanent_exclusion_requests}",
        ),
    )
    ...
    if allow_exhaustive:
        ...
        release = "EXHAUSTIVE_FINAL: all ECASP G1-G11 gates passed."
    elif not g1:
    ...
    return ECASPResult(
        ...
        object_counts={
            ...
            "non_dilution_compliant": preserved,
            "permanent_exclusion_requests": permanent_exclusion_requests,
        },
