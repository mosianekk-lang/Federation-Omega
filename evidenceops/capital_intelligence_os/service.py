...
class CapitalIntelligenceService:
    """Provider-neutral application facade; durable deployments should use DurableAutopilotRuntime."""
    def __init__(self) -> None:
        self.autopilot = Autopilot(); self.gravity = GravityEngine(); self.financing = FinancingStressEngine(); self.deals: dict[str, DealLifecycle] = {}
    ...
    def deal(self, deal_id: str) -> DealLifecycle:
        if deal_id not in self.deals: self.deals[deal_id] = DealLifecycle(deal_id)
        return self.deals[deal_id]
    def health(self) -> Mapping[str, Any]:
        return {"service":"EvidenceOps Capital Intelligence OS","release":"0.2.0-durable","authority_ceiling":"A1_INTERNAL","live_trading":False,"external_financial_effects":False,"learning_chain_valid":self.autopilot.ledger.verify(),"deal_count":len(self.deals)}
