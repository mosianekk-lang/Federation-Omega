...
def verify() -> dict[str, object]:
    ...
    probes=[AdapterProbe(f"verify-{i}","verification-provider",True,(control,),f"provider-receipt:{control}",now.isoformat()) for i,control in enumerate(preflight.required_controls(intent))]
    complete=preflight.evaluate(intent,claims,probes,now=now)
    gate=ProductionQualificationGate()
    gate_required=set(gate.required_controls(DeploymentIntent("STAGING","UNBOUND",private_mna_enabled=True,market_intelligence_enabled=True)))
    compiled_controls={e.control_id for e in complete.provider_evidence}
    checks={
        ...
        "compiled_provider_evidence_validated": bool(complete.provider_evidence) and all(e.state.value=="VERIFIED" for e in complete.provider_evidence),
    ...
    return {"passed":all(checks.values()),"release":"1.0.0-rc4","maturity":"PROVIDER_BINDING_READY","checks":checks}
