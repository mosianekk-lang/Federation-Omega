...
class OperationsFabric:
    ...
    def classify_failure(self, error: str) -> dict:
        ...
        if not text.strip():
            category = "NONE"
        elif any(token in text for token in ("permission", "unauthor", "forbidden")):
            category = "AUTHORITY"
            ...
            category = "INTEGRITY"
        elif any(token in text for token in ("quota", "billing", "budget")):
            category = "RESOURCE"
        else:
            category = "UNKNOWN"
        return {
    ...
    def choose_repair(self, failure: dict) -> dict:
        ...
        return {
            ...
            "automatic": category in {"NONE", "TRANSIENT", "RESOURCE", "CONTRACT", "INTEGRITY"},
        }
    ...
    def retirement_decision(self, metrics: dict) -> dict:
        triggers: list[str] = []
        if float(metrics.get("value_score", 1.0)) < 0.3:
            triggers.append("LOW_VALUE")
        if float(metrics.get("failure_rate", 0.0)) > 0.25:
            triggers.append("HIGH_FAILURE_RATE")
        if bool(metrics.get("replacement_ready", False)):
    ...
    def maintenance_cycle(
        ...
        started = self.heartbeat(system_id, "MAINTENANCE_RUNNING")
