...
class TruthGridGuard:
    ...
    This is intentionally strict.  It implements the control classes repeatedly
    identified by TruthGrid Genesis audits: integrity-scope leakage, unstable Gmail
    attachment handles, positional row targeting, malformed release receipts,
    revision drift, unsupported role/authority promotion and generic release prose.
    """
    ...
    def _enforce_integrity_manifest_only(self, intent: MutationIntent) -> None:
        ...
        if populated:
            raise TruthGridViolation(
                "RAW_INTEGRITY_OUTSIDE_MANIFEST:" + ",".join(sorted(populated))
            )
        ...
        for key, value in intent.values.items():
            if "ATTACHMENT_ID" in key.upper() and value not in (None, ""):
                ...
                if classification not in {
                    "EPHEMERAL_ACQUISITION_HANDLE",
                    "CONNECTOR_RUNTIME_HANDLE",
                }:
        ...
        if not source_classes.intersection(
            {
                ...
                "PROVIDER_NATIVE_AUTHORITY_RECORD",
            }
        ):
    ...
    def _enforce_receipt_schema(self, intent: MutationIntent) -> None:
        ...
        missing = [
            field_name
            for field_name in self.receipt_required_fields
            if field_name not in intent.values
        ]
        if missing:
            raise TruthGridViolation(
                "RELEASE_RECEIPT_SCHEMA_MISSING:" + ",".join(missing)
            )
        if not intent.values.get("Gate_Receipt_ID"):
            raise TruthGridViolation("RELEASE_RECEIPT_KEY_REQUIRED")
        if str(intent.values.get("Independent_Second_Pass", "")).upper() not in {
            "YES",
            "TRUE",
            "PASS",
            "N/A",
    ...
    def mission_lock(parent: Mission, requested_mission_id: str) -> MissionLockDecision:
        ...
        override = any(
            (
                parent.explicit_user_override,
                parent.external_deadline_override,
                parent.legal_preservation_override,
                parent.safety_override,
            )
        )
        if override:
    ...
    def assert_downstream_allowed(parent: Mission, requested_mission_id: str) -> None:
        ...
        if decision == MissionLockDecision.STAY_ON_FOUNDATION and requested_mission_id != parent.mission_id:
            raise TruthGridViolation(
                "FOUNDATION_COMPLETION_LOCK:" + parent.mission_id
            )
