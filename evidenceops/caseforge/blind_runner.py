...
def _find_control_leaks(value: Any, *, path: str = "$") -> tuple[str, ...]:
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            ...
            child = f"{path}.{key}"
            if normalized in RESERVED_CONTROL_KEYS:
                leaks.append(child)
...
class IsolatedBlindRunner:
    ...
    This is a deterministic isolation contract. It does not claim process,
    container, provider or model isolation until a provider-bound canary and
    independent readback prove those stronger states.
    """
