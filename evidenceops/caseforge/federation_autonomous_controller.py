...
class RuntimeAttestation:
    ...
    def qualifies_stage16(self) -> bool:
        self.validate()
        if self.activation_kind not in {
            ActivationKind.NEW_CHAT,
            ActivationKind.RESTORED_CHAT,
            ActivationKind.NON_CHAT_RUNTIME,
        }:
            return False
        ...
        if self.activation_kind == ActivationKind.RESTORED_CHAT and not self.mission_restore_ref.strip():
            return False
        return bool(self.capability_twin_ref.strip())
...
class EvolutionGovernorBridge:
    """Pre-gate that forwards qualified candidates to the existing EvolutionGovernor."""
    ...
    def _score(self, metrics: Mapping[str, float]) -> float:
        ...
        return sum(
            self.weights[name] * self._clamp(metrics.get(name, 0.0))
    ...
    def evaluate(
        ...
        *,
        ...
        hard = tuple(
            ...
            if self._clamp(candidate_metrics.get(metric, 0.0))
            < self._clamp(baseline_metrics.get(metric, 0.0))
        )
        ...
        decision = (
            "FORWARD_TO_EXISTING_EVOLUTION_GOVERNOR"
            if not reasons
            else "REJECT_PREPROMOTION"
        )
...
class CapabilityFormationEngine:
    ...
    def plan(self, gap: CapabilityGap) -> CapabilityFormationPlan:
        ...
        slug = "".join(
            ch if ch.isalnum() else "-" for ch in gap.missing_role.upper()
        ).strip("-")
...
class AutonomousMaturityDominanceController:
    """Decision loop only. This source does not create a durable scheduler."""
    ...
    def choose_weakness(self, signals: Sequence[WeaknessSignal]) -> WeaknessSignal | None:
        if not signals:
            return None
        return max(
            (signal.validate() for signal in signals),
            key=lambda item: (item.priority, item.system_id),
        )
    ...
    def decide(
        ...
        *,
        ...
        maturity = self.maturity_gate.classify(
            completed_through=completed_through,
            proof=maturity_proof,
        )
