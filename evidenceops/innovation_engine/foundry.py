...
from .foundry_registry import FoundryInnovationRegistry
...
class EvidenceOpsAlgorithmFoundry(FoundryLearningMixin):
    ...
    def __init__(
        ...
        *,
        ...
        self.learning = LearningFabric(
            self.workspace / "learning", policy_path=learning_policy_path
        )
        self.registry = FoundryInnovationRegistry(self.workspace / "innovation.db")
        self.algorithm_ledger = AlgorithmLedger(
            self.workspace / "algorithm_evolution.db"
        )
        self.evolution = EvolutionGovernor(self.algorithm_ledger)
        self.catalog_path = (
            Path(catalog_path)
            if catalog_path
            else Path(__file__).with_name("algorithm_catalog.json")
        )
        self.catalog = json.loads(self.catalog_path.read_text(encoding="utf-8"))
        self.catalog_sync = self.registry.synchronise_catalog(self.catalog)
        if not self.catalog_sync["complete"]:
            raise RuntimeError("algorithm catalog and innovation registry are inconsistent")
