...
Diagnose observed chat/runtime failure signatures, preserve mission state and
emit a bounded, idempotent recovery route. CFRE does not claim control of the
ChatGPT UI, OpenAI service, a browser/network, or third-party providers. It
makes EvidenceOps/Federation workflows resume-safe when those layers fail.
"""
...
# A generic timeout/stall is often only a symptom. On equal evidence scores,
# prefer the more specific causal surface so recovery uses the right safety
# semantics (for example readback-before-retry for a timed-out tool write).
CLASS_PRIORITY: dict[str, int] = {
    ...
    "CONTEXT_PRESSURE": 75,
    "CLIENT_RESOURCE_FAILURE": 70,
    "USER_INTERRUPTION": 65,
    "SERVER_GENERATION_FAILURE": 60,
    "STALL_TIMEOUT": 40,
    "UNKNOWN_CHAT_FAILURE": 0,
}

# Signatures are conservative evidence signals, not proof of a provider root
# cause. The engine preserves all matched candidates for later diagnosis.
...
def classify_failure(event: dict[str, Any]) -> tuple[FailureCandidate, ...]:
    ...
    candidates.sort(
        key=lambda item: (
            -item.score,
            -CLASS_PRIORITY.get(item.failure_class, 0),
            item.failure_class,
        )
    )
...
def build_checkpoint(event: dict[str, Any], previous: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    checkpoint["idempotency_key"] = _stable_sha256(
...
def _steps_for(failure_class: str, event: dict[str, Any], checkpoint: dict[str, Any]) -> tuple[RecoveryStep, ...]:
    del checkpoint  # checkpoint is already persisted/bound by the caller; keep signature explicit.
    steps: list[RecoveryStep] = [
        RecoveryStep(
            ...
            True,
            True,
            rationale="Never let a chat/transport failure erase directive, proof state, or next action.",
        ),
        RecoveryStep(
            2,
            "VERIFY_MISSION_NOT_COMPLETE",
            "OBJECTIVE_COMPLETION_GUARD",
            True,
            True,
            rationale="A failed or stalled response is not a completion receipt.",
        ),
    ]
    ...
    if failure_class in {
        ...
        steps.extend(
            [
                RecoveryStep(
                    3,
                    "RETRY_SAME_ATOMIC_ACTION",
                    "BOUNDED_EXPONENTIAL_BACKOFF",
                    True,
                    True,
                    rationale="Replay only the unfinished atomic action using its idempotency key.",
                ),
                RecoveryStep(
                    4,
                    ...
                    rationale="Do not regenerate already-proven work.",
                ),
            ]
        )
        ...
        steps.extend(
            [
                RecoveryStep(
                    3,
                    "COMPACT_CONTINUITY_STATE",
                    "LOSSLESS_CONTROL_SUMMARY",
                    True,
                    True,
                    rationale="Preserve directives, decisions, proof pointers, blockers and next action while shedding conversational bulk.",
                ),
                RecoveryStep(
                    4,
                    "START_FRESH_EXECUTION_CONTEXT",
                    "CHATBRIDGE_HANDOFF",
                    True,
                    True,
                    True,
                    True,
                    "Resume from the compact checkpoint rather than reconstructing the mission from memory.",
                ),
            ]
        )
        ...
        steps.extend(
            [
                RecoveryStep(
                    ...
                    True,
                    True,
                    rationale="A timed-out write may have succeeded; verify before replay.",
                ),
                RecoveryStep(
                    4,
                    "ISOLATE_FAILED_TOOL_LANE",
                    "ANTI_STALL_ROUTING",
                    True,
                    True,
                    rationale="Continue unaffected lanes while the provider/tool lane is repaired.",
                ),
                RecoveryStep(
                    5,
                    "DISCOVER_EQUIVALENT_AUTHORIZED_ROUTE",
                    "CAPABILITY_RESOLUTION",
                    True,
                    True,
                    True,
                    True,
                    "Use an alternate connected executor only when authority and semantics are equivalent.",
                ),
            ]
        )
        ...
        steps.extend(
            [
                ...
                RecoveryStep(
                    4,
                    "REGENERATE_OR_REATTACH_FROM_CANONICAL_SOURCE",
                    "ARTIFACT_RECOVERY",
                    True,
                    True,
                    True,
        ...
        steps.extend(
            [
                RecoveryStep(
                    3,
                    "PRESERVE_PENDING_ACTION_WITHOUT_REPLAY",
                    "FAIL_CLOSED",
                    True,
                    True,
                    rationale="Do not consume or duplicate an action under uncertain authority.",
                ),
                RecoveryStep(
                    4,
                    "RECHECK_AUTHORIZED_EXECUTOR",
                    "CAPABILITY_RESOLUTION",
                    True,
                    True,
                    True,
                    True,
                    "Resume automatically only when the connected surface exposes valid authority again.",
                ),
            ]
        )
        ...
        steps.extend(
            [
                ...
                RecoveryStep(
                    4,
                    "RESUME_ON_HEALTHY_EXECUTION_CONTEXT",
                    "CHATBRIDGE_HANDOFF",
                    True,
                    True,
                    True,
        ...
        steps.append(
            RecoveryStep(
                3,
                "WAIT_FOR_NEXT_USER_TURN_THEN_RESUME",
                "USER_CONTROLLED_RESUME",
                False,
                True,
                rationale="Do not override an explicit user stop or cancel.",
            )
        )
    else:
        steps.extend(
    ...
    if event.get("atomic_action") is False or event.get("payload_large") is True:
        ...
        steps.append(
            RecoveryStep(
                ...
                True,
                True,
                rationale="Reduce the blast radius of another interruption.",
            )
        )
    return tuple(steps)
...
def evaluate_failure(
    ...
    *,
    ...
    if mission_packet is not None:
        ...
        mission_complete = event.get("mission_complete") is True
        completion_claim_permitted = mission_complete and event.get("completion_proof") is True

    steps = _steps_for(primary.failure_class, event, checkpoint)
    route_exhausted = event.get("route_exhaustion_proven") is True
    ...
    digest_body["generated_at"] = "<TIME>"
    digest_checkpoint = dict(checkpoint)
    digest_checkpoint["captured_at"] = "<TIME>"
    digest_body["checkpoint"] = digest_checkpoint
...
def main(argv: list[str] | None = None) -> int:
    ...
    mission = (
        json.loads(Path(args.mission).read_text(encoding="utf-8"))
        if args.mission
        else None
    )
    previous = None
