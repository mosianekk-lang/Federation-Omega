...
#
# This script deploys only the LAUNCH-NOW / ZERO-POSSESSION service boundary.
# It deliberately does NOT require an IDV provider, biometric processing,
# document storage, malware/DLP, Cloud SQL, commissioner supply, or public
# unauthenticated traffic. It must run only inside an already-authorised Google
# Cloud execution plane. Source existence is not provider execution proof.
...
[[ "$CONFIRMATION" == "DEPLOY_ECERTIFY_LAUNCH_NOW_ZERO_TRAFFIC_CANARY" ]] || {
  echo "Explicit Launch-Now canary confirmation required" >&2
  exit 3
}
[[ "$SERVICE" != "architron9" ]] || { echo "Refusing reserved service target" >&2; exit 4; }

for cmd in gcloud docker python3 curl; do
  command -v "$cmd" >/dev/null || { echo "$cmd unavailable" >&2; exit 5; }
done
...
# Keep the canary authenticated and at zero production traffic. The tag gives the
# exact candidate revision a provider-native URL for bounded validation while the
# service's ordinary traffic assignment remains unchanged.
...
base_url = status.get("url") or ""
ready_revision = status.get("latestReadyRevisionName") or ""
tag_url = ""
for item in status.get("traffic") or []:
    if item.get("tag") == tag:
        tag_url = item.get("url") or ""
        break
if not base_url or not tag_url or not ready_revision:
    raise SystemExit("provider readback missing base URL, canary tag URL or ready revision")
print(base_url, tag_url, ready_revision)
PY
)

TOKEN="$(gcloud auth print-identity-token --audiences="$BASE_URL")"
HEALTH="$(curl --fail --silent --show-error \
  -H "Authorization: Bearer $TOKEN" "$TAG_URL/health")"
...
assert health.get("version") == "0.9.0", health
assert health.get("mode") == "launch_now", health
assert health.get("zero_possession_integrity_receipts") is True, health
assert health.get("identity_provider_required_for_launch") is False, health
...
ISSUED="$(curl --fail --silent --show-error \
  ...
  -d "$ISSUE_PAYLOAD" \
  "$TAG_URL/v1/integrity/receipt/issue")"
...
if missing:
    raise SystemExit(f"issued receipt missing fields: {missing}")
print(json.dumps({key: receipt[key] for key in sorted(required)}))
PY
)"
VERIFIED="$(curl --fail --silent --show-error \
  -H "Authorization: Bearer $TOKEN" \
  -H "content-type: application/json" \
  -d "$VERIFY_PAYLOAD" \
  "$TAG_URL/v1/integrity/receipt/verify")"
...
print("ECERTIFY_LAUNCH_NOW_RECEIPT_ROUNDTRIP_VERIFIED")
PY
...
print(json.dumps({
    "receipt": "ECERTIFY-ZA-LAUNCH-NOW-ZERO-TRAFFIC-CANARY-VERIFIED",
    "project": sys.argv[1],
    "region": sys.argv[2],
    "service": sys.argv[3],
    "ready_revision": sys.argv[4],
    "image": sys.argv[5],
    "canary_tag": sys.argv[6],
    "service_url": sys.argv[7],
    "canary_url": sys.argv[8],
    "integrity_key_id": sys.argv[9],
    "mode": "launch_now",
    "zero_possession": True,
    "identity_provider_required": False,
    "document_bytes_transmitted": False,
    "traffic_promoted": False,
    "public_unauthenticated": False,
    "proof_scope": "zero-traffic provider canary only; not public production launch"
}, sort_keys=True))
PY
