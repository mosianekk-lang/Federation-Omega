...
class BehaviorRegistry:
    ...
    def promote(self, candidate_id: str, incumbent: ExperimentEvidence, candidate: ExperimentEvidence, *, minimum_utility_delta: float = 0.03) -> PromotionDecision:
        ...
        if candidate.cohort_hash != incumbent.cohort_hash or candidate.holdout_hash != incumbent.holdout_hash:
            return PromotionDecision("RETAIN", self._champion.bundle_id, "UNMATCHED_COHORT_OR_HOLDOUT")
        if not candidate.evidence_refs or not candidate.regression_green or not candidate.shadow_complete or not candidate.canary_complete:
...
class SemanticDoneContract:
    ...
    def evaluate(self, observed: Mapping[str, bool]) -> tuple[bool, tuple[str, ...]]:
        missing = tuple(
            ...
            if not observed.get(predicate, False) or not self.proof_refs.get(predicate)
        )
