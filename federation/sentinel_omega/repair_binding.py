    ...
    def match(self, *, incident_class: str, required_capabilities: tuple[str, ...]) -> tuple[RepairRunbook, ...]:
        ...
        return tuple(
            sorted(
                matches,
                key=lambda item: (
                    _AUTHORITY_RANK[item.max_authority],
                    item.external_effect,
                    item.risk,
                    item.runbook_id,
                ),
            )
    ...
    def bind(
        ...
        *,
        ...
        matches = registry.match(
            ...
            required_capabilities=tuple(action.required_capabilities),
        )
        if not matches:
            return BoundRepairPlan(
        ...
        state = (
            RepairBindingState.BOUND_PROVIDER_EXECUTION_REQUIRED
            if action.external_effect
            else RepairBindingState.BOUND_INTERNAL_PROOF_REQUIRED
        )
        return BoundRepairPlan(
            state=state,
            action_id=action.action_id,
