...
class Repository:
    ...
    def _init_schema(self) -> None:
        with self.connect() as conn:
            conn.executescript(
                ...
                CREATE UNIQUE INDEX IF NOT EXISTS idx_cognitive_binding_unique
                  ON proof_records(matter_id, mission_id, subject_id)
                  WHERE proof_type='COGNITIVE_BINDING';
            ...
            workflow_columns = {
                str(row[1]) for row in conn.execute("PRAGMA table_info(workflows)").fetchall()
            }
            if "lease_generation" not in workflow_columns:
                conn.execute(
                    "ALTER TABLE workflows ADD COLUMN lease_generation INTEGER NOT NULL DEFAULT 0"
                )
    ...
    def fetch_one(self, query: str, params: Sequence[Any] = ()) -> sqlite3.Row | None:
        with self.connect() as conn:
            row: sqlite3.Row | None = conn.execute(query, tuple(params)).fetchone()
            return row
