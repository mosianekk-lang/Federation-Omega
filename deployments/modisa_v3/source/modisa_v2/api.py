...
from .nonce_stores import NonceStore, NonceStoreUnavailable
from .schemas import (
...
from .webhook_auth import SecretResolver, WebhookAuthError
...
def create_app(
    ...
    services: Services = build_services(
        settings,
        webhook_secret_resolver=webhook_secret_resolver,
        webhook_nonce_store=webhook_nonce_store,
    )
    ...
    async def health() -> HealthResponse:
        ...
        if (
            settings.webhook_auth_configured
            and services.webhook_nonce_backend_status != "ready"
        ):
            limitations.append("Webhook nonce backend is not provider-ready")
        ready = (
            ...
            and audit_ready
            and (
                not settings.webhook_auth_configured
                or services.webhook_nonce_backend_status == "ready"
            )
        )
        return HealthResponse(
            ...
            webhook_auth_configured=settings.webhook_auth_configured,
            webhook_secret_resolver_kind=services.webhook_secret_resolver_kind,
            webhook_secret_provider_proven=services.webhook_secret_provider_proven,
            webhook_nonce_store_kind=services.webhook_nonce_store_kind,
            webhook_replay_scope=services.webhook_replay_scope,
            webhook_nonce_backend_configured=services.webhook_nonce_backend_configured,
            webhook_nonce_backend_proven=services.webhook_nonce_backend_proven,
            webhook_nonce_backend_status=services.webhook_nonce_backend_status,
    ...
    async def update_workflow(workflow_id: str, request: WorkflowStateRequest, p: Principal) -> WorkflowRecord:
        ...
        if record is None:
            raise HTTPException(status_code=404, detail="Workflow not found")
        enforce(p, AccessRequirement(roles=frozenset({"OWNER", "COUNSEL", "ANALYST", "CONNECTOR"}), scopes=frozenset({"workflow:execute"})), record.matter_id)
        return services.workflows.update_state(
            workflow_id, request.worker_id, request.lease_generation, request.state
        )
    ...
    async def fail_workflow(workflow_id: str, request: WorkflowFailureRequest, p: Principal) -> WorkflowRecord:
        ...
        if record is None:
            raise HTTPException(status_code=404, detail="Workflow not found")
        enforce(p, AccessRequirement(roles=frozenset({"OWNER", "COUNSEL", "ANALYST", "CONNECTOR"}), scopes=frozenset({"workflow:execute"})), record.matter_id)
        return services.workflows.fail(
            workflow_id,
            request.worker_id,
    ...
    @app.post("/v2/webhooks/modisa")
    async def authenticate_modisa_webhook(
        request: Request,
        key_id: Annotated[str, Header(alias="X-Modisa-Key-Id")],
        timestamp: Annotated[int, Header(alias="X-Modisa-Timestamp")],
        nonce: Annotated[str, Header(alias="X-Modisa-Nonce")],
        signature: Annotated[str, Header(alias="X-Modisa-Signature")],
    ) -> dict[str, object]:
        authenticator = services.webhook_authenticator
        if authenticator is None:
            raise HTTPException(status_code=503, detail="MODISA webhook authentication unavailable")
        ...
        if content_length is not None:
            ...
            if declared_length < 0 or declared_length > settings.webhook_max_body_bytes:
                raise HTTPException(status_code=413, detail="Webhook body exceeds policy")
        chunks: list[bytes] = []
        observed_length = 0
        async for chunk in request.stream():
            observed_length += len(chunk)
            if observed_length > settings.webhook_max_body_bytes:
                raise HTTPException(status_code=413, detail="Webhook body exceeds policy")
            chunks.append(chunk)
        body = b"".join(chunks)
        try:
            receipt = authenticator.verify(
                key_id=key_id,
                timestamp=timestamp,
                nonce=nonce,
                signature=signature,
            ...
            services.webhook_nonce_backend_proven = False
            services.webhook_nonce_backend_status = "unavailable"
            raise HTTPException(
                status_code=503, detail="MODISA webhook replay protection unavailable"
            ) from None
        except WebhookAuthError as exc:
            raise HTTPException(status_code=401, detail=str(exc)) from exc
        return {
            "status": "authenticated_no_dispatch",
            "auth": receipt.model_dump(mode="json", by_alias=True),
            "external_effects": 0,
        }
