...
class SovereignLegalRuntime:
    ...
    def _build(self) -> None:
        ...
        for role, focus in COUNCIL_PROMPTS.items():
            tools: list[Any] = (
                ...
                if role == CouncilRole.AUTHORITY_VERIFIER
                else []
            )
    ...
    def _build_session(self, session_id: str) -> Any:
        ...
        if backend == "sqlalchemy":
            ...
            return SQLAlchemySession.from_url(
                session_id, url=self.settings.session_database_url.get_secret_value()
            )
    ...
    async def run(self, request: MissionRequest) -> MissionResponse:
        ...
        leased_workflow = self.s.workflows.lease(
            workflow.workflow_id, "runtime-inline", lease_seconds=3600
        )
        try:
            self._build()
            ...
            self.s.workflows.complete(
                ...
                lease_generation=leased_workflow.lease_generation,
                state={
