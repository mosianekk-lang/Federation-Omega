...
class ProofBoundReleaseEngine:
    ...
    def _validate_proofs(self, request: ReleaseRequest, records: list[ProofRecord]) -> tuple[list[str], dict[ProofType, list[ProofRecord]]]:
        ...
        if request.requirements.legal_analysis and law_checks and not any(
            bool(record.payload.get("current_primary_authority_checked")) for record in law_checks
        ):
            failures.append("current primary authority check not proven")

        treatment = by_type.get(ProofType.AUTHORITY_TREATMENT, [])
        if request.requirements.current_law_required and treatment and not any(
            bool(record.payload.get("amendment_and_subsequent_treatment_checked"))
    ...
    def evaluate(self, request: ReleaseRequest, actor_id: str = "release-engine") -> ReleaseResult:
        ...
        verified_proof_ids = [record.proof_id for record in records] + [release_proof.proof_id]
        receipt_payload = {
