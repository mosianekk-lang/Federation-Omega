...
Status runtime_open(const std::wstring&,Runtime*);void runtime_close(Runtime*);Status task_digest(const TaskEnvelope*,Digest256*);Status append_event(Runtime*,EventType,ByteSpan);Status project_runtime(Runtime*,TaskSecurityProjection*);Status nonce_claim(Runtime*,const Digest256&,const Digest256&);Status permit_issue(Runtime*,const PreparedAction*,ExecutionPermit*);Status permit_consume(Runtime*,const ExecutionPermit*);int run_mvs_canary(const std::wstring&);int run_mvs_crash_child(const std::wstring&);
}
