...
def source_only_canary_decision(spec: CreativeCanarySpec) -> CreativeCanaryDecision:
    """Return the next gate for a valid source contract without inventing effect authority."""
...
def evaluate_creative_canary(
    ...
    *,
    ...
    """Evaluate evidence for one bounded media canary without inheriting authority.

    `provider_effect_authority_bound` and `finite_spend_authorized` are execution-time
    evidence supplied from an external authority/control plane; neither can be granted by
    this module or the source contract itself.
    """
