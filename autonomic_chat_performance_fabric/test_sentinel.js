...
assert.equal(canary.schema, "FACPF-BROWSER-CANARY-2");
assert.equal(canary.mode, "SHADOW_OBSERVE_ONLY");
assert.equal(canary.activation.authorizedEdgeHookPresent, false);
assert.equal(canary.activation.defaultState, "ADAPTER_REQUIRED");
assert.deepEqual(canary.routeContract.acceptedInputModes, ["ACTIVE", "PASSIVE"]);
assert.equal(canary.routeContract.rejectDerivedHealthAsInputMode, true);
