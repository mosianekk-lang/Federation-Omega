...
The process-coordinated journal-safe atomic authority-snapshot control plane is
the supported live-authority entry point. Live authority requires a
provider-native snapshot, monotonic durable acceptance, exact action-level
binding, atomic prepare/commit/rollback, a durable recovery bundle, atomically
published transaction events, and one provider-process coordination lock that
serializes startup recovery with every consequential action. Earlier control
planes remain exported for historical regression and mock-provider conformance.
"""
